...

{
 ...
 "themes": [
   {"event":"<co se PRÁVĚ děje ve světě>","thesis":"<řetězec: událost → dopad → kdo vydělá/prodělá>","tickers":"<TICKERy z reportu, kterých se teze týká>"}
 ],
 "follow_up": [
   {"ticker":"<TICKER>","action":"Drž|Utni|Posuň stop na $X|Vezmi zisk","note":"<1 věta proč>"}
 ]
}
(pole "follow_up" vracej JEN pokud v zadání dostaneš seznam otevřených tipů)
""".strip()

PROMPT = f"""Jsi aktivní trading desk pro KRÁTKODOBÉ příležitosti. Vytvoř DNEŠNÍ ranní report v češtině.

ZDROJE (čerpej přes web search a KŘÍŽOVĚ ověřuj — víc zdrojů = míň chyb):
...
POSTUP (SHORA DOLŮ — od světa k akciím):
1) SVĚT: nejdřív zjisti, co se PRÁVĚ TEĎ děje ve světě — geopolitika (konflikty, sankce,
   dohody, volby), centrální banky a sazby, ropa/energie, AI a technologie, regulace,
   dodavatelské řetězce, komodity (CNBC, Reuters, FRED). Vyber 2–4 NEJSILNĚJŠÍ
   události/trendy tohoto okamžiku.
2) TEZE: z každé události odvoď investiční tezi — řetězec UDÁLOST → dopad → sektor →
   kdo vydělá/prodělá. Zapiš je do pole "themes" (event, thesis, tickers).
3) Podle tezí vyber {NUM_STOCKS} titulů (dny–týdny, VYŠŠÍ riziko, napříč odvětvími).
   Pro KAŽDÝ ověř AKTUÁLNÍ cenu a cíl analytiků MINIMÁLNĚ ze DVOU zdrojů
   (např. Yahoo + StockAnalysis/Finviz). Když se cena liší o víc než ~3 %, vezmi
   konzervativnější a zmiň to v analýze. Zjisti i P/E, beta, 52t max/min. Katalyzátor
   KAŽDÉHO titulu musí odkazovat na konkrétní světovou událost/tezi z bodu 2.
   NIC si nevymýšlej — co nenajdeš, "n/a".
4) upside = cíl/cena - 1. recommendation: "Koupit" (upside>=10 % a Buy/Strong Buy, nebo
...
5) FOLLOW-UP: pokud na konci zadání dostaneš seznam OTEVŘENÝCH TIPŮ z minulých reportů,
   ke KAŽDÉMU vrať položku v poli "follow_up": akce "Drž" / "Utni" / "Posuň stop na $X" /
   "Vezmi zisk" + 1 věta proč (podle aktuální ceny vs stop/cíl a jestli teze stále platí).
6) {SCHEMA_HINT}
...
def generate(followup_ctx: str = "") -> dict:
    ...
    # drahé, pomalé (i 27 min) a komplikace s container_id. Ceny stejně ověřujeme
    # deterministicky přes Yahoo, takže snippety z hledání bohatě stačí.
    tools = [
    ...
    # Server-side nástroje běží ve smyčce; při 'pause_turn' pokračujeme.
    for _ in range(8):
        kwargs = dict(
            model=MODEL,
            max_tokens=20000,
        ...
        if container_id:
            kwargs["container"] = container_id   # nutné při pokračování s code execution
        with client.messages.stream(**kwargs) as stream:
            msg = stream.get_final_message()

        if getattr(msg, "container", None):
...
def main():
    ...
    # výsledkovka: vyhodnoť otevřené tipy (živé ceny) a připrav follow-up kontext
    followup_ctx = ""
    try:
        import tips_tracker
        tips_tracker.evaluate()
        followup_ctx = tips_tracker.followup_context()
    except Exception as e:
        print(f"tips_tracker (evaluate) selhal: {e}", file=sys.stderr)

    print(f"Generuji report ({MODEL}, {NUM_STOCKS} firem)…", file=sys.stderr)
    report = generate(followup_ctx)
    ...
    try:
        import tips_tracker
        report = tips_tracker.enrich_followup(report)
    except Exception as e:
        print(f"tips_tracker (enrich) selhal: {e}", file=sys.stderr)
    ...
    # zaloguj nové nákupní tipy do výsledkovky
    try:
        import tips_tracker
        tips_tracker.log_new(report, today)
        tips_tracker.evaluate()
    except Exception as e:
        print(f"tips_tracker (log) selhal: {e}", file=sys.stderr)
