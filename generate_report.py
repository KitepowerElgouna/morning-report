...
POSTUP:
1) Hledáš krátkodobé (dny až týdny), VYŠŠÍ riziko. Použij web search a najdi {NUM_STOCKS}
   titulů, které se PRÁVĚ TEĎ hýbou nebo mají blízký katalyzátor (výsledky, zpráva,
   cenový průraz, silné/slabé momentum) — napříč odvětvími, ať je mix. Pro každý zjisti
   AKTUÁLNÍ reálná čísla: cenu, cíl analytiků, rating, P/E, beta, 52týdenní max/min,
   a co s ním právě hýbe. NIC si nevymýšlej — co nenajdeš, dej "n/a".

2) Spočítej upside = cíl/cena - 1. recommendation: "Koupit" (upside>=10 % a Buy/Strong Buy,
   nebo >=15 %), "Sledovat" (<=-8 % nebo Sell), jinak "Držet".
   NAVÍC u KAŽDÉHO titulu povinně urči trade plán: krátkodobou akci, STOP-LOSS (cena, kde
   vystoupit při ztrátě — povinné!), krátkodobý cíl, a velikost pozice. Řiď riziko:
   žádná pozice > ~5 % "satelitu", vždy stop-loss. Seřaď podle síly příležitosti/momenta.

3) {SCHEMA_HINT}

DŮLEŽITÉ:
- Výstupem tvé poslední zprávy je POUZE ten jeden JSON objekt (žádný text okolo).
- Čísla reálná z webu; cíle = odhady (NE záruka); jde o KRÁTKODOBÉ RIZIKOVÉ tipy pro
  "satelitní" peníze, které lze ztratit — NE pro jádro portfolia. Toto uveď v summary
  spolu s připomínkou: drž stop-lossy a velikost pozice, časté přetrádování škodí.
"""
